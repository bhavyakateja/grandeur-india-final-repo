...
let sessionRestorePromise: Promise<SessionRestoreResult | null> | null = null;
...
function messageFor(status: number, data: unknown) {
  if (data && typeof data === "object") {
    ...
    if ("errors" in data && Array.isArray(data.errors)) {
      ...
      const message = data.errors.find(
        (item) =>
          item &&
          typeof item === "object" &&
          "message" in item &&
...
/**
 * Restores an existing browser session.
 *
 * This is intentionally separate from apiRequest():
 * - a first page load has no in-memory access token;
 * - the refresh-token cookie is HttpOnly and cannot be inspected by JS;
 * - the refresh endpoint already returns both the user and a new access token;
 * - therefore the app does not need to call /auth/me just to discover the user.
 */
