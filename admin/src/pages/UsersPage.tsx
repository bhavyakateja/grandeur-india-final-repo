      ...
      <form onSubmit={save} className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="text-sm">
              ...
              <select
                ...
                className="mt-2 h-9 w-full rounded-md border px-3 disabled:bg-slate-100"
                value={role}
            ...
            <label className="text-sm">
              ...
              <select
                className="mt-2 h-9 w-full rounded-md border px-3"
          ...
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={verified} onChange={(e) => setVerified(e.target.checked)} /> Verified account
          </label>
        </div>
  ...
  if (e && !u)
    return (
      <>
        ...
        <div className="mt-4">
          <ErrorState message={e} retry={load} />
        </div>
      </>
    );
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            ...
            <Button variant="destructive" disabled={!u.isActive || busy} onClick={() => void deactivate()}>
              <UserX />
              Deactivate
            </Button>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border bg-card p-6 space-y-4">
          ...
          <div className="flex justify-between">
            ...
            <span>{formatDate(u.lastLoginAt ?? undefined)}</span>
          </div>
        </div>
        <div className="rounded-xl border bg-card p-6">
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            {[
              ...
              ["Wishlists", u._count?.wishlists ?? 0],
            ].map(([a, b]) => (
              <div key={a} className="rounded-lg bg-slate-50 p-4 text-center">
  ...
  return (
    <div>
      <PageHeader
        ...
        description={`${total} accounts. Administrators can manage account states and roles.`}
      />
      {error && (
        <div className="mb-4">
                ...
                {data.map((u) => (
                  <tr key={u.id} className="border-t">
                    ...
                    <td className="p-3 text-right">
                      <Button size="sm" variant="outline" onClick={() => setSelected(u.id)}>
                        <Eye />
                        View
                      </Button>
                    </td>
