...
export default function ReviewsPage() {
  ...
  const setReviewStatus = async (review: Review, next: ReviewStatus) => {
    try {
      await adminApi.updateReviewStatus(review.id, next);
      // In-place local state update without re-fetching all reviews
      setData((prev) =>
        prev.map((r) => (r.id === review.id ? { ...r, status: next } : r)),
      );
    } catch (e) {
      setError(e instanceof Error ? e.message : "Unable to moderate review.");
    }
  };
  ...
  const remove = async (review: Review) => {
    ...
    try {
      ...
      // In-place local state removal
      setData((prev) => prev.filter((r) => r.id !== review.id));
  ...
  return (
    <div>
      <PageHeader
        ...
        description={`${total} customer reviews. Approve, reject, or remove content.`}
      />
      {error && (
        <div className="mb-4">
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="font-medium">{r.product.name}</h2>
                ...
                <div className="flex gap-2">
                  {r.status !== "APPROVED" && (
                    <Button
                      size="sm"
                      onClick={() => void setReviewStatus(r, "APPROVED")}
                    >
                      Approve
                    </Button>
                  )}
                  {r.status !== "REJECTED" && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void setReviewStatus(r, "REJECTED")}
                    >
                      Reject
                    </Button>
                  )}
