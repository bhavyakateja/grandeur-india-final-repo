...
function CouponForm({
  ...
  const [f, setF] = useState<FormState>({
    ...
    minimumOrderAmount:
      initial?.minimumOrderAmount == null
        ? ""
        : Number(initial.minimumOrderAmount),
    maximumDiscount:
      initial?.maximumDiscount == null
        ? ""
        : Number(initial.maximumDiscount),
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        f.type === "FIXED" &&
        f.maximumDiscount !== "" &&
        f.maximumDiscount !== undefined
      ) {
        throw new Error("Maximum discount is only applicable to percentage coupons.");
      }
      const input = {
        ...f,
        ...
        minimumOrderAmount:
          f.minimumOrderAmount === "" ? undefined : Number(f.minimumOrderAmount),
        maximumDiscount:
          f.maximumDiscount === "" ? undefined : Number(f.maximumDiscount),
      ...
      const saved = initial
        ? await adminApi.updateCoupon(initial.id, input)
        : await adminApi.createCoupon(input);
      onSaved(saved);
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-4">
      ...
      >
        <div className="flex justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Coupon rules are enforced by the checkout backend.
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <select
              ...
              onChange={(e) =>
                setF({
                  ...
                  maximumDiscount:
                    e.target.value === "FIXED" ? "" : f.maximumDiscount,
                })
              }
            >
          ...
          <div>
            ...
            <Input
              ...
              min="0"
              step="0.01"
              value={f.maximumDiscount}
              disabled={f.type === "FIXED"}
              onChange={(e) =>
                setF({
                  ...f,
                  maximumDiscount:
                    e.target.value === "" || f.type === "FIXED"
                      ? ""
        ...
        <div className="mt-6 flex justify-end gap-2 border-t pt-5">
          ...
          <Button disabled={busy}>{busy ? "Saving…" : "Save coupon"}</Button>
        </div>
      </form>
    </div>
  );
...
export default function CouponsPage() {
  ...
  const load = async (signal?: AbortSignal) => {
    try {
      setLoading(true);
      setError("");
      setData(await adminApi.coupons(signal));
    } catch (e) {
  ...
  const remove = async (c: Coupon) => {
    ...
    try {
      ...
      // In-place local removal without re-fetching entire list from DB
  ...
  return (
    <div>
      <PageHeader
        title="Coupons"
        description="Manage promotional rules used by checkout."
        action={
      ...
      {loading ? (
        ...
        <Empty
          title="No coupons"
          description="Create a coupon to make one available at checkout."
        />
      ) : (
                ...
                {data.map((c) => (
                  <tr key={c.id} className="border-t">
                    ...
                    <td className="p-3 text-xs">
                      {formatDate(c.startsAt ?? undefined)}
                      <br />
                      {formatDate(c.expiresAt ?? undefined)}
                    </td>
                    <td className="p-3">
      ...
      {editing && (
        <CouponForm
          ...
          onSaved={(savedCoupon) => {
            ...
            setData((prev) => {
              const exists = prev.some((item) => item.id === savedCoupon.id);
              return exists
                ? prev.map((item) =>
                  item.id === savedCoupon.id ? savedCoupon : item,
                )
                : [savedCoupon, ...prev];
            });
          }}
